...
"""
Adaboost Regression model using a Decision Tree Regressor for a walk-forward based approach
"""
...
if run == 'custom':
    system = SystemComponents(feature_space='OHLCTAMV', feature_engineering='', processor='')
    model_name = 'ABR'
    ...
    for t in market_etfs:
        ...
        if not system.feature_engineering == '' and not system.feature_engineering == 'PCA':
            ...
            print(t, 'features:', x_proc.shape[1])

        DT = DecisionTreeRegressor(criterion='friedman_mse', max_depth=6, min_samples_split=2, max_features=8)
        ...
        for i in range(train_start, len(x_diff)):
            ...
            x_test = pd.DataFrame(MAS.transform(x_test), columns=x_train.columns, index=x_diff.index[i:i+1])
